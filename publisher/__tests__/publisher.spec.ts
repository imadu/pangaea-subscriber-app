...
import { Subscription } from "../src/data/subscriptions/subscriptions.model";
...
describe("Publisher controller", () => {
  it("should receive subscriptions", async () => {
    ...
    const res = await request(app).post(`/subscribe/${topic}`).send({
      url,
    });
    ...
    const sub = await Subscription.findOne({ topic });
  ...
  it("should publish to registered subscribers", async () => {
    ...
    await Subscription.create({ topic, subscribers: [url] });
